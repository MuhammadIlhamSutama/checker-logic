...
# ─── THREAD LOCKS (SISTEM ANTREAN) ───────────────────────────────────────────
cache_io_lock = threading.Lock()
analysis_locks = {}
locks_manager_lock = threading.Lock()
...
# Hash: Diperbarui menjadi fusi 3 CTI (VT, CTX, OTX)
W_HASH_VT  = 0.34
W_HASH_CTX = 0.33
W_HASH_OTX = 0
...
def normalize_otx(raw):
    if not raw or any(x in str(raw).lower() for x in ["none", "error"]):
        return 0.0

    raw_lower = str(raw).lower()

    if "malicious" in raw_lower or "high risk" in raw_lower:
        return 1.0

    # Cuckoo score — handles medium risk dan low risk sekaligus
    score_match = re.search(r'score:([\d\.]+)', raw_lower)
    if score_match:
        return min(float(score_match.group(1)) / 10.0, 1.0)
...
def get_threat_analysis(file_hash: str, src_ip: str, dest_ip: str,
    ...
    with locks_manager_lock:
        if cache_key not in analysis_locks:
            analysis_locks[cache_key] = threading.Lock()
        item_lock = analysis_locks[cache_key]
